...
def _estimate_network_cost(plan, partition_scheme, num_nodes, alias_dict):
  if 'Plans' in plan:
    children_costs = list()
    for subnode in plan['Plans']:
      children_costs.append(_estimate_network_cost(subnode, partition_scheme, num_nodes, alias_dict))
    current_cost = estimate_network_cost_for_single_plan_node(plan, partition_scheme, num_nodes, alias_dict) 
    return sum(children_costs) + current_cost
  else:
    # cost = # rows * width
    return 0

def estimate_network_cost_for_single_plan_node(plan, partition_scheme, \
  ...
  if 'Join' in name:
    ...
    if name == 'Hash Join': # hash left/right/semi join
      cost = estimate_hash_join_cost(plan, sub_costs, partition_scheme, num_nodes, alias_dict)
...
def estimate_hash_join_cost(plan, sub_costs, partition_scheme, \
  num_nodes, alias_dict):
  ...
  # is_left_replicated = partition_scheme.is_replicated(left_table)
  # is_right_replicated = partition_scheme.is_replicated(right_table)
  ...
  if (is_left_partitioned and is_right_partitioned):
      # (is_left_partitioned and is_right_replicated) or \
      # (is_left_replicated and is_right_partitioned):
    # okay!
    if PRINT_TOGGLE:
  ...
  #   return left_cost
    ...
    #     # add left
    #     if not is_left_replicated:
    #       partition_scheme.add_scheme(left_table, replicate=True)
    #   else:
    #     # add right
    #     if not is_right_replicated:
  ...
  for root,dir,files in os.walk(plans):
    for file in files:
      ...
      with open(os.path.join(root,file)) as fp:
        plan=json.load(fp)
        costs.append(_estimate_network_cost(plan[0][0][0]['Plan'],db.partition_scheme,0,0))
